function [y] = my_sub_sep_icwt(P, f, f_span)
...
% calculate normalized factor C_phi
phi_sampling_rate = f(2)-f(1);      % the frequencies f(i) are normalized ! (in [0,1]...)
xx = [-1+phi_sampling_rate:phi_sampling_rate:1];    % --> we have 2/phi_sampling_rate symbols
...
for tau=1:N_T
    for fi=f_use_index(1):f_use_index(end)   
        ...
        P_fi(fi) = P(fi,:)*conj(phi.'); % integral over tau: represented in this scalar product
    end
