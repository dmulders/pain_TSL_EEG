...
function [handle,Zi,grid,Xi,Yi] = my_topoplot(Values,loc_file,varargin)
...
DEFAULT_COLORMAP = 'jet' ; % ADDED
gb = [0 0];
COLORARRAY2 = { [gb 0] [gb 1/4] [gb 2/4] [gb 3/4] [gb 1] };

disp_title_topoplot = 0 ; % DM 2018
...
% 
  if isstr(loc_file)
      [tmpeloc labels Th Rd indices] = my_readlocs( loc_file);
...
if ~strcmpi(STYLE,'blank') % if draw interpolated scalp map
 if ~strcmpi(STYLE,'grid') %  not a rectangular channel grid
  ...
  %
  if strcmpi(VERBOSE,'on')
      disp(['Warning: When plotting pvalues in topoplot, use option ''conv'' ',...
          'to minimize extrapolation effects']);
  end
  ...
  elseif strcmp(STYLE,'both')  % plot interpolated surface and surface contours
      if strcmp(SHADING,'interp')
       tmph = surface(Xi*unsh,Yi*unsh,zeros(size(Zi))-0.1,Zi,...
          ...
          % other way to add the pmask (only ok in white):
          %Zi_tmp = Zi ; Zi_tmp(round(ZiC)==0) = NaN ;
          %tmph = surface(Xi-delta/2,Yi-delta/2,zeros(size(Zi))-0.1,Zi_tmp,...
          %       'EdgeColor','none','FaceColor',SHADING);
    ...
    if ~PMASKFLAG
        ...
        % ZiC is the interpolated pmask (hence between 0 and 1)
        ZiC(find(ZiC > 0.5 )) = NaN ; 
        % NaN values are not considered in contourf
        ZiC_tmp = ZiC ; ZiC_tmp(~isnan(ZiC)) = 1 ; 
        [cls, chs] = contourf(Xi,Yi,min(amin-1000,-10000).*ZiC_tmp,0,'k') ; 
        % assign first color of color map to NS data ("0" in the mask)
        % 0 contour line (of specified color) inside the masked zone
        
        %[cls chs] = contourf(Xi,Yi,ZiC,0,'k'); % INITIAL VERSION: 0 assign
        %to color mask
        
        subh = get(chs, 'children') ; 
        for indsubh = 1:length(subh)
            numfaces = size(get(subh(indsubh), 'XData'),1); 
            set(subh(indsubh), 'FaceVertexCData', ones(numfaces,3), ... %%%%%%%%%%%%%%%%%
                'Cdatamapping', 'direct', 'facealpha', 0.5, 'linewidth', 2);
        end;
    end;
    handle = tmph;                                   % surface handle
    for h=chs, set(h,'color',CCOLOR); end % color of the contour lines
    warning on;
  %
  %%%%%%%%%%%%%%%%%%%%%%%% Else plot map only %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  %
...
%
  ...
  if (strcmp(ELECTRODES,'labelpoint') |  strcmp(ELECTRODES,'numpoint')) && disp_title_topoplot
    text(-0.6,-0.6, ...
...
try,
    if strcmpi(STYLE,'blank') % if mark-selected-channel-locations mode
        ...
        for kk = 1:length(1:length(x))
            if abs(Values(kk))
                if strcmp(PLOTDISK,'on') %PLOTDISK
                    ...
                    % hexagons on top:
                    %hp2 = disk(y(kk),x(kk),0.04, tmpcolor, 0, 360, 20);
                    % disk(X, Y, radius, colorfill, oriangle, endangle, segments)
                end;
            end;
        end
    end
...
%
%%%%%%%%%%%%% Modify ylim s.t. head plot is seen entirely %%%%%%%%%%%%%%%%%%%%%%%%
% 
% (NEW -- especially for the nose)
yl = get(gca, 'ylim') ; 
