...
function [eloc, labels, theta, radius, indices] = my_readlocs( filename, varargin ); 
...
if isstr(filename)
   ...
   elseif strcmp(lower(g.filetype(1:end-1)), 'polhemus') | ...
       ...
       if strcmpi(g.filetype, 'chanedit')
           array = my_loadtxt( filename, 'delim', 9, 'skipline', ...
               g.skiplines, 'blankcell', 'off', 'verbose', 'off');
       else
...
function array = load_file_or_array( varname, skiplines );
    ...
    if exist( varname ) == 2
        array = my_loadtxt(varname,'verbose','off','skipline',skiplines,'blankcell','off');
