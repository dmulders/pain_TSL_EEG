function [n_data, n_data_conf, est_conf_tmp, est_p1_tmp, IO_p1_tmp, ...
    IO_MAP_p1_tmp, IO_conf_tmp, trials_resp_TP, trials_resp_conf, s, ...
    all_RT, all_RT_conf, p1_mean, conf_p1] = ...
    load_extract_ratings(fn_test, remove_missed_resp, in)
% Load the rating data and extract useful information.
%
% When behavioral questions are asked to the subject, we ask them to
% estimate:
% - proba of next stimulus
% - confidence in this estimation.
% When learning TP, the confidence is therefore the one for the TP from the
% previous I received --> ok thanks to 
% "out.p1_sd      = GetConditionalValue(out.p1g2_sd,out.p2g1_sd,in.s);" in
% IdealObserver.m.
...
p1_mean = out_SO.p1_mean ; p1_sd = out_SO.p1_sd ; p1_MAP = out_SO.p1_MAP ; 
conf_p1 = -log2(p1_sd) ;
% p1_mean = p1_mean
% p1_MAP = p1_MAP
% p1_sd = p1_sd
...
IO_p1_tmp       = make_row_vector(p1_mean(trials_resp_TP)) ;
IO_MAP_p1_tmp   = make_row_vector(p1_MAP(trials_resp_TP)) ;
IO_conf_tmp     = make_row_vector(conf_p1(trials_resp_conf)) ; 
