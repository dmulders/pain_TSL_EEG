function [varargout] = BernoulliNoJump(s, priorpA, method, MemParam, OutPut, prior_data)
...
%   * N_used: #observations counted to estimate posterior
...
if strcmp(method, 'slow')
    ...
    s_hat          = zeros(1, L);
    pmY            = zeros(L, 1);
    N_used         = (pNA+pNB).*ones(L,1) ; % #observations used for each estimate 
    
    % FOR EACH OBSERVATIONS
    for k = 1:L
        
        % Compute event count, including the current observation
        [~, ~, ~, ~, ~, NA, NB] = CountEventInMemory(s(1:k), MemParam, prior_data);
        N_used(k) = N_used(k) + NA + NB ; 
    ...
    % Compute event count, including the current observation
    [~, ~, ~, ~, ~, NA, NB] = CountEventInMemory(s, MemParam, prior_data);
    N_used = pNA + pNB + NA + NB ; 
