function [varargout] = MarkovNoJump(s, t, priorpAgB, priorpBgA, method, MemParam, OutPut, AboutFirst,prior_data)
...
%       o [NEWW] {'DecayF', 1}: use the largest weights for the first
%       observations.
%       o [NEWW] {'LimitedF', 1}: use the largest weights for the first
%       observations.
%       o [NEWW] {'Gaussian', 1}: Gaussian weights instead of exp(-k/w)
...
%   * N_used: number of observations used to perform inference. 
...
if nargin <= 5
    MemParam = [];
elseif iscell(MemParam) && any(length(MemParam) == [2 4 6 8 10 12])
...
if nargin<9
    prior_data = struct('factorN1', 1, 'factorN2', 1) ; 
end
...
if strcmp(method, 'slow')
    ...
    N_used         = zeros(L,1) ; % #observations used for each estimate 
    N_used(s==1)   = pNAgA + pNBgA ; 
    N_used(s==2)   = pNAgB + pNBgB ; 
    ...
    for k = 2:L
        ...
        [NBgA, NAgB, NAgA, NBgB, s1] = CountEventInMemory(s(1:k), MemParam, prior_data);
        if s(k) == 1
            N_used(k) = N_used(k) + NAgA + NBgA ; 
        else
            N_used(k) = N_used(k) + NAgB + NBgB ; 
        end
        ...
        if strcmpi(AboutFirst, 'WithoutFirst')
            ...
            [windowed, windowedF, MemSpan, MemSpanF, leaky_int, leakyF_int, ...
                MemDecay, MemDecayF, Weight, Gaussian] = get_memParams(MemParam) ; 
            
            if ~(windowedF && MemSpanF>0) && (windowed && k >= MemSpan)
                % idem in CountEventInMemory, k should be >= MemSpan
                p1 = ComputeLikelihood1stEvent(MemDecay, s1, MemSpan, nt, t, ...
                    MemDecayF, Gaussian, leaky_int, leakyF_int, Weight);
            else
                p1 = ComputeLikelihood1stEvent(MemDecay, s1, k, nt, t, MemDecayF, ...
                    Gaussian, leaky_int, leakyF_int, Weight);
            end
        end
        ...
        if nargout >=5 || any(strcmp(OutPut, 'pmY')) % compute only if necessary because it is time consuming
    ...
    % Count transitions including the current one
    [NBgA, NAgB, NAgA, NBgB, s1] = CountEventInMemory(s, MemParam, prior_data);
    ...
    if strcmp(AboutFirst, 'WithoutFirst')
        ...
        [windowed, windowedF, MemSpan, MemSpanF, leaky_int, leakyF_int, ...
        MemDecay, MemDecayF, Weight, Gaussian] = get_memParams(MemParam) ;
        if (windowed && L >= MemSpan)
            p1 = ComputeLikelihood1stEvent(MemDecay, s1, MemSpan, nt, t, MemDecayF, ...
                Gaussian, leaky_int, leakyF_int, Weight);
            % (MemDecay, s1, k, nt, t, MemDecayF, Gaussian)
        else
            p1 = ComputeLikelihood1stEvent(MemDecay, s1, L, nt, t, MemDecayF, ...
                Gaussian, leaky_int, leakyF_int, Weight);
        end
    ...
    if s(end) == 1
        N_used = pNAgA + pNBgA + NAgA + NBgA ; 
    else
        N_used = pNAgB + pNBgB + NAgB + NBgB ; 
    end
...
if isempty(OutPut)
    ...
    if nargout >= 4, varargout{4} = N_used ; end
    if nargout >= 5, varargout{5} = pmY;      end
    if nargout >= 6, varargout{6} = predA;    end
    if nargout >= 7, varargout{7} = predA_sd; end
else
    if iscell(OutPut)
...
function p1 = ComputeLikelihood1stEvent(MemDecay, s1, k, nt, t, MemDecayF, ...
    Gaussian, leaky_int, leakyF_int, Weight)
...
pA = (ones(nt, 1)*t) ./ (ones(nt, 1)*t + t'*ones(1,nt)); %pA = p(A|B)/(p(A|B)+p(B|A))
pB = (t'*ones(1, nt)) ./ (ones(nt, 1)*t + t'*ones(1,nt));%pB = p(B|A)/(p(A|B)+p(B|A))
...
%p1 = p1.^(exp(-(1/MemDecay)*(k-1))); 
% -1: e.g. k=2: we have in memory events 1 & 2; 1 is 1 time back in the past.
exponent = 0 ; use_exp = 0 ; 
bias_weight = 0 ; % cfr CountEventInMemory
if leaky_int && MemDecay>0 %MemDecay < Inf
    use_exp = 1 ; 
    exponent = exponent + Weight.*exp(-(k-1+bias_weight)/MemDecay) ; 
end
if leakyF_int && MemDecayF>0 %MemDecayF < Inf
    use_exp = 1 ; 
    exponent = exponent + exp(-bias_weight/MemDecayF) ; 
end

if use_exp
    p1 = p1.^exponent ;
    %p1 = p1.^(exp(-(1/MemDecayF))+exp(-1/MemDecay*k)) ;
end
% MemDecayF: largest weight for first obs
% MemDecay: for consistency with CountEventInMemory
% exponent because the count is an exponent in beta distrib
...
% Note that p(y|Markov,t)*p(t) was computed above (in ComputePosterior), but up to a scaling
...
if strcmpi(AboutFirst, 'WithoutFirst')
    ...
    pmY = (1/(nt-1))^2*(nansum(nansum(LL .* Prior))); % integral
