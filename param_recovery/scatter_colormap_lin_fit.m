...
% cfr scatter_with_colormap
% used in test_SO_inference (colormap) and CN_analyze_ratings (lin fits).
%
% data_struct
...
if add_lin_fit
    ...
    hold on ; 
    yL = get(gca,'YLim') ; 
    xval_poly = linspace(xL(1)+eps, xL(2)+eps, 30) ; 
...
if add_identity
    ...
    yL = get(gca,'YLim') ;
    plot(xL+eps, xL+eps, ':','LineWidth',2, 'Color', 0.5*ones(1,3)) ; hold on
    xlim(xL) ;
end
if log_axes
   set(gca,'XScale','log', 'YScale', 'log') ;   
