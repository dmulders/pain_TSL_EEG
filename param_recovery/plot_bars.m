...
% used in CN_fit_on_ratings (exceedance proba).
%
% data_mat
...
gr_labels = strcat('gr ', strsplit(num2str(1:n_gr))) ; 
...
if use_colormap && n_gr==1
    ...
    if view_3D
        ...
        %caxis(caxis_val) ; 
        colormap(c_bars) ;
        legend(h, gr_labels, 'location', 'best') ; 
    end
...
if view_3D || use_colormap
    ...
    set(gca,'YGrid', 'off', 'XGrid','off','XMinorGrid','off', ...
        ...
        'Xtick',1:n_pos, 'XtickLabel', x_descr, 'box', 'off') ; %, 'FontName', 'Arial') ;
