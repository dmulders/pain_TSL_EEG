...
%   - "p": a Nx1 (Bernoulli) specifying p(1) in each chunk or 
%          a Nx2 (Markov) specifying p(1|2) and p(2|1) in each chunk.
%Usage:
...
% adapted to start with random intensity. 
...
% col 1: chunk starts: col 2: chunk ends
...
for c = 1:Nchunks
    ...
    elseif numel(p(c,:)) == 2
        ...
        for k = 1:N
            if k == 1
                %RndGen = 1 + rand - p1; % OLD version: start each chunk in state 2.
                RndGen = rand - p1;
            else
                RndGen = rand - T(s(idx(k)-1)); 
            end
